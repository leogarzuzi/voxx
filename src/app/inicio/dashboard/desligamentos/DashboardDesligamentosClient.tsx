...
export default function DashboardDesligamentosClient({
  ...
  return (
    <main className="min-h-screen min-w-0 bg-[#11141b] p-8 text-slate-100">
      <section className="overflow-hidden rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_14%_0%,rgba(239,68,68,0.2),transparent_32%),linear-gradient(135deg,#242833_0%,#171a23_58%,#10131a_100%)] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.32)]">
      ...
      {filtroAtivo && (
        <div className="mt-6 flex flex-wrap items-center gap-3 rounded-[22px] border border-red-300/20 bg-red-300/[0.07] px-4 py-3">
          <span className="text-sm font-semibold text-red-100">
            Seleção ativa:
          </span>
          {mesSelecionado && (
            <span className="rounded-full border border-red-300/25 bg-red-300/10 px-3 py-1 text-xs font-bold text-red-100">
              Mês {mesSelecionado}
            </span>
          )}
          {divisaoSelecionada && (
      ...
      <section className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <div className="rounded-[28px] border border-white/10 bg-[#171a23] p-6 shadow-[0_22px_70px_rgba(0,0,0,0.28)]">
          ...
          <div className="voxx-scrollbar mt-6 overflow-x-auto rounded-[22px] border border-white/10 bg-[#202532]">
